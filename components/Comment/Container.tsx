...
const Comments = ({ comments, handleResponseSubmit }: CommentProps): JSX.Element => {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLSelectElement>) => setSortBy(event.currentTarget.value);
  ...
  return (
    <>
      ...
      {!!comments.length &&
        sortContent(comments, sortBy).map(({ comment, id, date, userName, colorID, reply }): JSX.Element => {
          return (
            <Comment
              reply={reply}
              comment={comment}
